...
def process_recipe(recipe):
    try:
        ...
        if steps_key in recipe and recipe[steps_key] and len(recipe[steps_key]) >= 3:
            ...
            nonsense = random.choice(NONSENSE) if random.random() < 0.5 else ""
            recipe['steps'] = [
                template[0].format(ingredients=' and '.join(ingredients_list)),
                template[1].format(method=method.lower(), equipment=primary_equipment),
                template[2].format(extra=extra_text) + f" {nonsense} {chaos_events[0]} {disaster} {insult}"
            ]
